...
## Last updated
- **Date:** 2026-08-06
- **Agent:** Desktop (branch `feat/monthly-guides`, off `main`, not yet merged)

---

## Desktop Agent — 2026-08-06 (/when-to-go: month-by-month guides for September, October, November)

### What was done this session
Pulled `main` and confirmed `feat/seo-sitemap` and `feat/motion-and-species` were both merged before starting (they weren't — merged them first, see the merge-integration entry below, then branched `feat/monthly-guides` off the result).

- **Route + data structure**: New `/when-to-go` section. `app/when-to-go/data.ts` holds a `MONTHS` array (all 12 months, lightweight fields for the season-at-a-glance table) and `MONTH_GUIDE_DETAILS` (full structured fields — weather, target species, hotspots, notes, one-day plan — for months with a written guide), following the `app/birds/data.ts` pattern so the sitemap and index page read from one source instead of a duplicated list. Index page: year overview, a 12-row arriving/leaving/peaking table (built from facts already established elsewhere on the site — the existing `south-texas-birding-calendar` article, hotspot `bestSeason` fields, species status fields — not invented), and a month-card grid where unwritten months render as a muted, `aria-disabled` card rather than a link. Shared `MonthPanel`/`MonthNav` components (same reasoning as `HeroFrame`/`BirdNav` for species pages). Verified the commit boundary itself was link-safe: shipped this commit with `hasGuide: false` for all 12 months (including Sept/Oct/Nov) so the sitemap and index didn't reference not-yet-existing pages, then flipped to `true` in the next commit alongside the actual page files — checked the sitemap output at each boundary to confirm no dead entries.
- **Three month guides** (~830–890 words of prose each, all species/hotspot mentions linked and verified against real slugs before committing):
  - **September** — "Birding South Texas in September": Broad-winged Hawk kettle mechanics, peak timing (third week, historically Sep 18–25 — reused from the site's own already-published `fall-hawk-migration-south-texas` article rather than re-deriving), Bentsen's hawk tower, Hazel Bazemore mentioned as regional context only (Corpus Christi, ~100 mi away, explicitly not one of this site's 9 tracked hotspots — no anchor link fabricated for it), the Hook-billed Kite "kettle within the kettle," fall shorebirds at South Padre/Laguna Atascosa, and dawn-focused planning around **verified** McAllen-area climate data (92°F/74°F high/low, ~26 muggy days, wettest month of the year — sourced from WeatherSpark, cited below).
  - **October** — "The Turnover Month": hawks thinning, Painted Bunting/Vermilion Flycatcher/Aplomado Falcon starting to filter back in, Santa Ana's Oct–Apr best season opening, Clay-colored Thrush becoming more findable, real weather relief (87°F/68°F).
  - **November** — "Winter Season Opens": Whooping Crane arrival/settling at Aransas (reusing the site's own established "late October arrival, Nov–Mar viewing window" language and real population-recovery figures — 15 birds in 1941 to several hundred today — already on the whooping-crane species page), four hotspots (Santa Ana, Laguna Atascosa, Salineño, Resaca de la Palma) entering their best season simultaneously, the RGVBF festival in Harlingen, comfortable temperatures (79°F/61°F).
  - **Caught and fixed a real accuracy bug before committing**: had originally listed Black-bellied Whistling-Duck as a November target species, but that species' own "Best Time to See" text says it's most abundant April–October — the opposite of a November highlight. Cross-checked every target species against its own species page before finalizing; swapped it for American Oystercatcher (whose own page says October–March is its most productive period) and rewrote the affected paragraph.
- **Internal linking**: `/when-to-go` added to both the header nav (between Hotspots and Checklist — placed as a primary content pillar, not a footer-only afterthought, since it's comparable in importance to Birds/Hotspots rather than a utility page) and the footer nav. Species pages: added a link from "Best Time to See" to the matching guide **only where that species' own existing text already named the month** — checked all ~28 species whose page mentions Sept/Oct/Nov anywhere, and only linked the 11 whose Best Time section specifically calls out one of these three months (broad-winged-hawk, hook-billed-kite → September; piping-plover, reddish-egret, aplomado-falcon, painted-bunting, roseate-spoonbill, vermilion-flycatcher → October; mottled-duck, neotropic-cormorant, american-oystercatcher → November; whooping-crane → November). Skipped species like crested-caracara and white-tailed-hawk despite an earlier broad grep match, because their actual Best Time text is "Year-round" with no month tie-in — forcing a link there would've been exactly the spam the task warned against. `/hotspots` and `/news` each got one intro-paragraph sentence pointing to `/when-to-go`, not per-card/per-article spam. Four existing news articles that cover overlapping ground got reciprocal links: `fall-hawk-migration-south-texas` + `fall-shorebird-season-south-padre` → September, `south-texas-birding-calendar` → all three, `rgvbf-festival-2026` + `whooping-crane-record-count` → November.
- **Verification**: `npm run build` and `npm run lint` both clean throughout (checked at every commit boundary, not just the end). Wrote a small script cross-checking every `/birds/{slug}`, `/hotspots#{anchor}`, `/news/{slug}`, and `/when-to-go/{month}` href in the entire `app/` tree against the real data files — zero bad links found. Confirmed in the built HTML: all 4 new pages have correct absolute canonicals, all 3 month pages' OG images resolve to real files (reused Broad-winged Hawk / Painted Bunting / Whooping Crane species photos), and the sitemap grew from 78 → 82 URLs (the `/when-to-go` index at priority 0.9, three month pages at 0.8).

### Facts flagged for Rick to verify locally
- **September's "still hot" climate figures (92°F/74°F, humidity, rainfall) came from a live web search** (WeatherSpark's McAllen averages), not from anything already on this site — reasonably confident but worth a gut-check against your own on-the-ground experience, since McAllen (inland) can run a few degrees hotter/drier than the coastal parts of the Valley (Harlingen/Brownsville/South Padre) the guides also cover. Same for October (87°F/68°F) and November (79°F/61°F) figures.
- **"Third week of September, historically Sep 18–25" for Broad-winged Hawk peak** — this is not new; it's already stated on the existing `fall-hawk-migration-south-texas` article and `south-texas-birding-calendar` (Sep 15–25 there, slightly different range). Reused rather than re-verified independently — worth reconciling the two-day discrepancy between articles if it matters.
- **Hazel Bazemore's "hundreds of thousands" seasonal raptor count** — also reused from the existing `fall-hawk-migration-south-texas` article's "700,000+ in 2024" claim, not independently re-verified this session.
- **RGVBF festival timing** — deliberately kept vague ("held in Harlingen every November," no specific dates) since the existing `rgvbf-festival-2026` article itself says exact 2026 dates weren't yet announced at time of writing. Check whether dates have since been published and could be added.
- Nothing else was invented — no new specific dates, fees, or hours beyond what already exists elsewhere on the site.

### What's next
- **9 more months** — December through August — as the next content batch, same format (`MONTH_GUIDE_DETAILS` entry + page.tsx + flip `hasGuide`). The `MONTHS` array already has lightweight `arriving`/`leaving`/`peaking` data for all 12 from this session's calendar-article research, so the season-at-a-glance table won't need rework — just the full guide pages.
- Branch `feat/monthly-guides` has been pushed to origin but **not merged to `main`**.

---

## Desktop Agent — 2026-08-06 (merge integration)

Merged both same-day branches into `main`, in commit order: `feat/seo-sitemap` first (fast-forward, `main` hadn't diverged), then `feat/motion-and-species` (one conflict, in this file's own session write-up below — both sessions added a dated entry; resolved by keeping both, `feat/seo-sitemap`'s entry first since its commits landed first). No other files conflicted — the two branches touched disjoint areas (`app/news/data.ts` + `app/sitemap.ts` + per-page canonical/OG fixes vs. `app/birds/data.ts` + `birdsOrder.ts` + new species pages), confirmed by inspection, not just absence of conflict markers. Build and lint both pass clean on the merged `main`.
...
- Branch `feat/seo-sitemap` has been pushed to origin — **now merged into `main`**, see merge integration entry above.
...
- Branch `feat/motion-and-species` has been pushed to origin — **now merged into `main`**, see merge integration entry above.
